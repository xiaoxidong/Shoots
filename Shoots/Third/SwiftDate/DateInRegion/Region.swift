...
public struct Region: Decodable, Encodable, Equatable, Hashable, CustomStringConvertible {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let calId = try Calendar.Identifier(values.decode(String.self, forKey: .calendar))
        let tz = try (TimeZone(identifier: values.decode(String.self, forKey: .timezone)) ?? SwiftDate.defaultRegion.timeZone)
        let lc = try Locale(identifier: values.decode(String.self, forKey: .locale))
