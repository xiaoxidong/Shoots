...
public extension DateInRegion {
    ...
    func dateAfter(weeks count: Int, on weekday: WeekDay) -> DateInRegion {
        var result = dateByAdding(count, .weekOfMonth)
        if result.weekday == weekday.rawValue {
            return result
    ...
    {
        var result = dateAt(weekdayOrdinal: weekNumber, weekday: weekday, monthNumber: monthNumber)

        if result <= self {
            if let monthNum = monthNumber {
                result = dateAt(weekdayOrdinal: weekNumber, weekday: weekday,
                                monthNumber: monthNum, yearNumber: year + 1)
            } else {
                result = dateAt(weekdayOrdinal: weekNumber, weekday: weekday, monthNumber: month + 1)
            }
        }
