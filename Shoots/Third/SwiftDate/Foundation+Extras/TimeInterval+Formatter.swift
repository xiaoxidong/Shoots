...
public extension TimeInterval {
    struct ComponentsFormatterOptions {
        ...
        public func apply(toFormatter formatter: DateComponentsFormatter) {
            ...
            if let allowsFractionalUnits = allowsFractionalUnits {
                formatter.allowsFractionalUnits = allowsFractionalUnits
            }
            if let allowedUnits = allowedUnits {
                formatter.allowedUnits = allowedUnits
            }
            if let collapsesLargestUnit = collapsesLargestUnit {
                formatter.collapsesLargestUnit = collapsesLargestUnit
            }
            if let maximumUnitCount = maximumUnitCount {
                formatter.maximumUnitCount = maximumUnitCount
            }
            if let zeroFormattingBehavior = zeroFormattingBehavior {
