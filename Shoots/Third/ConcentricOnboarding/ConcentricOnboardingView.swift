...
public struct ConcentricOnboardingView<Content>: View, Animatable where Content: View {
    ...
    var size: CGPoint {
        if let window = UIApplication.shared.windows.filter({ $0.isKeyWindow }).first {
           let screenFrame = window.frame
            return CGPoint(x: screenFrame.width / 2 + 59, y: screenFrame.height / 2)
        } else {
    ...
    var size: CGPoint {
        if let window = NSApplication.shared.windows.first {
            return CGPoint(x: window.frame.width / 2 + 59, y: window.frame.height / 2)
        } else {
            return CGPoint(x: 4000, y: 4000)
        }
    }
    #endif
    private var shape: some View {
        AnimatableShape(progress: progress, radius: radius, limit: limit, direction: direction, size: size)
