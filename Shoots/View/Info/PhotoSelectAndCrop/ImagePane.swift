...
public struct ImagePane: View {
    ...
    private init(addPhotoText: String, changePhotoText: String, image: ImageAttributes, defaultImage _: UIImage, isEditMode: Binding<Bool>) {
        _imageAttributes = ObservedObject(initialValue: image)
        _isEditMode = isEditMode
        addPhotoButtonLabel = addPhotoText
        changePhotoButtonLabel = changePhotoText
    }
