...
import SwiftUI
    ...
    struct DNALoading: View {
        ...
        let firstBallColor: LinearGradient = .init(gradient: Gradient(colors: [firstC, secondC]), startPoint: .top, endPoint: .bottom)
        let secondBallColor: LinearGradient = .init(gradient: Gradient(colors: [firstC1, secondC2]), startPoint: .top, endPoint: .bottom)
    }
